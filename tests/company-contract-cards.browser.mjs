...
test('Company contracts load on show, render all statuses as cards, and paginate within the viewed company', { timeout: 90000 }, async () => {
  ...
  try {
    ...
    await evaluate(`window.failContractDetail = true; document.querySelector('[aria-label="Contract CONTRACT-1"] button[aria-label="Upload Contract Items"]').click()`)
    await until(() => evaluate(`document.body.textContent.includes('Unable to load contract detail')`))
    assert.equal(await evaluate(`!!document.querySelector('.contract-upload-modal')`), false)
    await evaluate(`window.failContractDetail = false; document.querySelector('[aria-label="Contract CONTRACT-1"] button[aria-label="Upload Contract Items"]').click()`)
    await until(() => evaluate(`!!document.querySelector('.contract-upload-summary')`))
    assert.equal(await evaluate(`document.querySelector('.contract-upload-summary').textContent.includes('CONTRACT-1')`), true)
    assert.equal(await evaluate(`document.querySelector('.contract-upload-summary').textContent.includes('Test Company')`), true)
    assert.equal(await evaluate(`document.querySelector('.contract-upload-modal').textContent.includes('Reload contract')`), false)
    assert.equal(await evaluate(`document.querySelectorAll('.contract-upload-modal .ant-select').length`), 0)
    await click('Cancel')
    await until(() => evaluate(`!document.querySelector('.contract-upload-modal')`))
